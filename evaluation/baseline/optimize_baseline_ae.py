...
import os
import argparse
from models.models import Conv1DClassifier, AlternatingLinearDropoutMLP, DeepMLP, AutoEncoder
...
def train_ae_conditional(model, model_classification, train_loader,tensor_x_test, tensor_y_test, criterion, criterion_classification, optimizer,optimizer_classifier, device, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            outputs, compression = model(inputs.to(device))  
        ...
        prob = model_classification(projection_test)
        accuracy, predicted_list, labels_list = test(model_classification, my_dataloader_test, device)
        accuracy_list.append(accuracy)
        

    return accuracy, accuracy_list, train_acc_list, reconstruction_loss, model, prob
...
if __name__ == '__main__':
    ...
    for i, (train_index, test_index) in enumerate(kfold.split(X, y)):
        ...
        prob_list_test, best_sizes_list, prediction_test, lables_test, performance_dict  = [], [], [], [], []
        ...
        for hidden_size in input_sizes:
            ...
            for i_sub, (train_index_sub, test_index_sub) in enumerate(kfold.split(X_train, y_train)):
                ...
                model = return_model(args.model_type, input_size = hidden_size)
                ...
                accuracy, accuracy_list, train_acc_list, reconstruction_loss, model, prob = train_ae_conditional(autoencoder, model, my_dataloader,tensor_x_test, tensor_y_test, criterion, criterion_classification, optimizer_ae,optimizer_classification, device, num_epochs=100)
