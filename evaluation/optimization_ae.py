...
class LSTMBinaryClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc(torch.relu(lstm_out)) 
...
class AutoEncoder(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        #self.fc2 = nn.Linear(input_size//2, hidden_size) 
        self.fc3 = nn.Linear(hidden_size, input_size)  
        
    def forward(self, x):

        out1 = torch.nn.functional.relu(self.fc1(x)) 
        #out1 = torch.nn.functional.relu(self.fc2(out1))  # Use the last hidden state
...
def train_ae_conditional(model, model_classification, train_loader,tensor_x_test, tensor_y_test, criterion, criterion_classification, optimizer,optimizer_classifier, device, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        my_dataset = MyDataset(projection_test,tensor_y_test) 
        my_dataloader_test = DataLoader(my_dataset, batch_size=32, shuffle = False) 
        
        accuracy = test(model_classification, my_dataloader_test, device)
        accuracy_list.append(accuracy)

    return total_loss, accuracy_list, train_acc_list, reconstruction_loss, model
...
if __name__ == '__main__':
    cmdline_parser = argparse.ArgumentParser('main logic used for optimizing ae feature number')
    ...
    for i, (train_index, test_index) in enumerate(kfold.split(X, y)):
        ...
        prob_list_test, lables_test, best_sizes_list, prediction_test, performance_dict = [], [], [], [],[]
    
        for hidden_size in input_sizes:
            prob_list, lables, average_prediction, prediction   = [], [], [], []
        

            for i_sub, (train_index_sub, test_index_sub) in enumerate(kfold.split(X_train, y_train)):
                ...
                accuracy, accuracy_list, train_acc_list, reconstruction_loss, model = train_ae_conditional(autoencoder, lstm_classifier, my_dataloader,tensor_x_test, tensor_y_test, criterion, criterion_classification, optimizer_ae,optimizer_classification, device, num_epochs=200)
                ...
                with torch.no_grad():
                    ...
                    tensor_x_train = normalize(tensor_x_train.cpu().detach())
                    tensor_x_test = normalize(tensor_x_test.cpu().detach())
        ...
        autoencoder = AutoEncoder(input_size, best_sizes).to(device)
        lstm_classifier = LSTMBinaryClassifier(best_sizes, best_sizes, num_layers=1).to(device)
        optimizer_ae = torch.optim.Adam(autoencoder.parameters(), lr=0.001) 
        optimizer_classification = torch.optim.Adam(lstm_classifier.parameters(), lr=0.001) 
        criterion = nn.MSELoss()
        criterion_classification = nn.BCEWithLogitsLoss()
